...
export default function AnalysisReportModal({ report, pet, onClose, currentLanguage = "en" }: AnalysisReportModalProps) {
  ...
  return (
    <div id="report-modal-backdrop" className="fixed inset-0 bg-black/40 backdrop-blur-md z-50 flex items-end justify-center p-0">
      ...
      <motion.div
        ...
        className="w-full max-w-md bg-white rounded-t-[2.5rem] p-6 shadow-2xl relative z-10 flex flex-col max-h-[85vh] overflow-hidden"
      ...
      >
        ...
        <div className="flex justify-between items-center mb-5">
          <div className="flex items-center gap-1.5">
            <Sparkles size={18} className="text-[#5856d6]" />
            <h3 className="text-base font-extrabold text-[#1c1c1e]">
              {t.AiAnalysis}: <span className="text-[#5856d6]">{pet.name}</span>
            </h3>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto space-y-5 pr-1 pb-4">
          ...
          <div className="bg-zinc-50 p-4 rounded-2xl border border-zinc-100 flex flex-col sm:flex-row items-center gap-4">
            {/* SVG Circular Gauge */}
            <div className="relative flex items-center justify-center shrink-0">
              ...
              <div className="absolute flex flex-col items-center">
                <span className="text-xl font-black font-mono leading-none">{report.healthScore}</span>
                <span className="text-[8px] text-zinc-400 uppercase tracking-widest font-bold">{t.Index}</span>
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <div className="bg-gradient-to-br from-rose-50/40 to-rose-50/10 p-4 rounded-2xl border border-rose-100/50 space-y-1.5">
            ...
            <div className="bg-gradient-to-br from-amber-50/40 to-amber-50/10 p-4 rounded-2xl border border-amber-100/50 space-y-1.5">
              ...
              {report.possibleDiseases && report.possibleDiseases.length > 0 ? (
                <div className="space-y-1">
                  <div className="flex flex-wrap gap-1">
                    {report.possibleDiseases.map((disease, idx) => (
                      <span
                        key={idx}
                        className="inline-block bg-amber-100/70 border border-amber-200/50 text-amber-800 text-[10px] font-bold px-2 py-0.5 rounded-md"
                      >
                        {disease}
                      </span>
                    ))}
                ...
                <div className="flex items-center gap-1.5 text-xs text-green-700 font-bold py-1">
          ...
          <div className="space-y-2">
            <h4 className="text-xs font-bold uppercase tracking-wider text-zinc-400 flex items-center gap-1.5 px-1">
            ...
            <div className="space-y-2">
              {report.findings.map((finding, idx) => (
                <div
                  key={idx}
                  className="bg-white p-3.5 rounded-2xl border border-zinc-100 shadow-3xs flex gap-3"
                >
                  ...
                  <div className="space-y-0.5 flex-1 min-w-0">
                    <div className="flex justify-between items-center gap-2">
                      <span className="text-xs font-bold text-[#1c1c1e] truncate">{finding.category}</span>
                    ...
                    <p className="text-xs text-zinc-500 leading-relaxed">{finding.details}</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="bg-indigo-50/25 p-4 rounded-2xl border border-indigo-100/50 space-y-2.5">
            ...
            <ul className="space-y-2">
              {report.recommendations.map((rec, idx) => (
                <li key={idx} className="flex gap-2 text-xs text-zinc-600 leading-relaxed font-medium">
                  <span className="h-5 w-5 bg-indigo-50 text-[#5856d6] font-bold text-[10px] rounded-full flex items-center justify-center shrink-0 mt-0.5">
                    {idx + 1}
                  </span>
                  <span>{rec}</span>
                </li>
          ...
          <div className="bg-amber-50/30 p-4 rounded-2xl border border-amber-100/50 space-y-1">
          ...
          <div className="bg-sky-50/30 p-4 rounded-2xl border border-sky-100/50 space-y-1">
          ...
          <div className="space-y-1.5">
            <span className="text-[11px] font-bold text-zinc-400 block px-1">{t.PhotoUsed}:</span>
        ...
        <div className="pt-4 border-t border-zinc-100 bg-white">
          <button
            onClick={onClose}
            className="w-full bg-[#1c1c1e] hover:bg-black text-white font-extrabold py-3.5 rounded-2xl transition-colors cursor-pointer text-center text-sm"
          >
            {t.CloseReport}
          </button>
        </div>
