...
export function ProgressView({ 
  ...
  const daysData: DayTrendItem[] = useMemo(() => {
    return Array.from({ length: 7 }).map((_, i) => {
      ...
      const dayLabel = isRu ? ruDays[d.getDay()] : enDays[d.getDay()];
  ...
  // Format date: "Сегодня", "Вчера", or "Д.М.Г." (DD.MM.YYYY)
  const displayDate = useMemo(() => {
    if (selectedDayOffset === 0) return isRu ? "Сегодня" : "Today";
    if (selectedDayOffset === 1) return isRu ? "Вчера" : "Yesterday";
    try {
      const d = new Date();
        ...
        >
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center text-center select-none pointer-events-none px-4 overflow-hidden">
            <AnimatePresence mode="wait">
              ...
              >
                {/* Date on top: Сегодня, Вчера, Д.М.Г. - matches diagram color */}
                <span 
                ...
                {selectedDayItem.hasCheck ? (
                  <>
                    ...
                    <span className="text-[14px] sm:text-[15px] font-medium text-[#8E8E93] tracking-tight mt-0.5">
                      {isRu ? "Состояние здоровья" : "Health status"}
                    </span>
                  </>
                ) : (
                  <span 
