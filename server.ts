...
async function resolveImageToBase64(imageInput: string): Promise<{ base64Data: string; mimeType: string }> {
  ...
  try {
    const fallbackPaths = [
      path.join(process.cwd(), "src/assets/images/pet_cat_1783674667219.jpg"),
      path.join(process.cwd(), "dist/assets/pet_cat_1783674667219.jpg")
    ];
...
Твои рекомендации должны носить консультативный характер и напоминать о важности очного обращения к ветеринару при серьезных отклонениях.
    ...
    return res.status(500).json({ 
      error: "Ошибка при анализе фотографии ИИ. Пожалуйста, попробуйте другую фотографию или повторите попытку позже.",
