...
const COMMON_SYMPTOMS = [
  { id: "normal", label: "✨ Профилактика / Всё отлично", category: "all" },
  { id: "lethargy", label: "💤 Вялость / Слабость", category: "all" },
  { id: "scratching", label: "🪰 Зуд / Расчесывание", category: "all" },
  { id: "eyes", label: "👁️ Покраснение глаз / Слезы", category: "all" },
  { id: "limping", label: "🐾 Хромота / Боль при движении", category: "all" },
  { id: "coat", label: "🧴 Выпадение шерсти / Проплешины", category: "cat_dog" },
  { id: "feathers", label: "🪶 Проблемы с перьями / Самосщип", category: "parrot" },
  { id: "cough", label: "💨 Кашель / Чихание", category: "all" },
  { id: "appetite", label: "🥣 Плохой аппетит", category: "all" }
];
...
const SYMPTOMS_TRANSLATIONS: Record<string, Record<string, string>> = {
  ...
  ru: {
    normal: "✨ Профилактика / Всё отлично",
    lethargy: "💤 Вялость / Слабость",
    scratching: "🪰 Зуд / Расчесывание",
    eyes: "👁️ Покраснение глаз / Слезы",
    limping: "🐾 Хромота / Боль при движении",
    coat: "🧴 Выпадение шерсти / Проплешины",
    feathers: "🪶 Проблемы с перьями / Самосщип",
    cough: "💨 Кашель / Чихание",
    appetite: "🥣 Плохой аппетит"
  },
  fr: {
...
const SCANNER_TRANSLATIONS: Record<string, {
  ...
  ru: {
    Analyze: "Проверить питомца",
    SymptomsFor: "Симптомы",
    UploadPhoto: "📸 Загрузить фото",
    ChangePhoto: "🔄 Сменить фото",
    Reset: "Сбросить",
    NewPet: "➕ Новый питомец",
    CheckSymptomsTitle: "Отметьте симптомы перед ИИ-анализом:",
    StartAiAnalysis: "Запустить анализ ИИ",
    Analyzing: "Идет ИИ-анализ...",
    ServerError: "Не удалось связаться с сервером AI."
  },
  fr: {
    Analyze: "Scanner",
...
export default function PetScanner({
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      console.error(error);
      setAnalysisError(error.message || "Не удалось связаться с сервером AI.");
    } finally {
  ...
  return (
    <div id="pet-scanner-section" className="w-full flex flex-col items-center">
      ...
      <AnimatePresence>
        {showSymptomsSheet && (
          <div className="fixed inset-0 bg-black/40 backdrop-blur-md z-50 flex items-end justify-center">
            <div className="absolute inset-0" onClick={() => setShowSymptomsSheet(false)}></div>
