...
export default function App() {
  ...
  const [activePetId, setActivePetId] = useState<string>("dog"); // Default centered dog
  ...
  const [healthLog, setHealthLog] = useState<HealthLog | null>(() => {
    ...
    const logKey = `pethealth_log_dog_${dateStr}`;
    ...
    return {
      ...
      waterIntake: 85,
      waterTarget: 150,
      medications: getDefaultMedications("dog"),
      pottyStatus: "normal",
      symptoms: [],
      notes: ""
    };
  ...
  const [reminders, setReminders] = useState<Reminder[]>(() => {
    const storedReminders = localStorage.getItem("pethealth_reminders");
    if (storedReminders) {
      try {
    ...
    return [
      { id: "1", title: "Ветеринарный осмотр", time: "14:00", completed: false },
      { id: "2", title: "Вакцинация", time: "11:00", completed: true },
    ];
  });
  ...
  const [completedPhotoDates, setCompletedPhotoDates] = useState<string[]>(() => {
    const stored = localStorage.getItem("pethealth_completed_photo_dates");
    if (stored) {
      try {
        return JSON.parse(stored);
      } catch (e) {}
  ...
  const handleUpdateLog = (updatedFields: Partial<HealthLog>) => {
    ...
    const updatedLog = { ...healthLog, ...updatedFields };
  ...
  const handleAnalysisSuccess = (report: AnalysisReport) => {
    ...
    const updatedReports = [report, ...reportsList];
    setReportsList(updatedReports);
    localStorage.setItem(`pethealth_reports_${activePetId}`, JSON.stringify(updatedReports));
  ...
  useEffect(() => {
    if (reportsList && reportsList.length > 0) {
      const reportDates = reportsList
        .map((r) => (r.createdAt ? r.createdAt.split("T")[0] : r.date))
        .filter(Boolean);
            ...
            {showLanguagePopup && (
              <div className="fixed inset-0 z-50 flex flex-col justify-end overflow-hidden">
                ...
                <motion.div
                  ...
                  className="relative w-full max-w-md mx-auto bg-white rounded-t-[64px] px-6 pt-10 pb-8 shadow-[0_-16px_48px_rgba(0,0,0,0.22)] flex flex-col z-10 select-none touch-none"
  ...
  return (
    <div className="w-full min-h-screen bg-[#f5f4fa] dark:bg-[#000000] text-[#1c1c1e] dark:text-[#f5f5f7] flex flex-col items-center pt-3.5 pb-28 px-3 select-none overflow-x-hidden relative transition-colors duration-250">
      ...
      {/* Floating iOS-style Bottom Navigation Bar with swipe & drag support */}
      <FloatingBottomNav
        activeTab={activeNavTab}
        onChangeTab={setActiveNavTab}
        onAddClick={() => setShowAppleActionSheet(true)}
      />

      {/* Apple System Style Action Sheet Modal for '+' Button */}
      <AppleActionSheet
        isOpen={showAppleActionSheet}
        onClose={() => setShowAppleActionSheet(false)}
