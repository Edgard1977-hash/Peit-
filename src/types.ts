export type PetType = 'cat' | 'dog' | 'parrot' | 'other';
...
export interface Reminder {
  ...
  title: string;
  time: string;
  type: 'medication' | 'water' | 'vet' | 'walk' | 'food';
  completed: boolean;
...
export interface AnalysisReport {
  ...
  statusLabel: string; // "Отличное" | "В норме" | "Требует внимания" | "Рекомендуется ветеринар"
  summary: string;
  findings: Array<{
    category: string;
...
/**
 * Returns color according to user rules:
 * ниже 50% – красный (#FF3B30)
 * ниже 75% – оранжевый (#FF9500)
 * ниже 90% – желтый (#FFCC00)
 * дальше (>= 90%) – яркий зеленый (#30D158)
 */
